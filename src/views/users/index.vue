      ...
      <v-row>
        ...
        <v-col cols="12" md="12" class="d-flex w-100">
          <DataTable
            ...
            :headers="headers"
            :total-records="getUserTotalRecord ? getUserTotalRecord : 0"
            :number-of-pages="getUserTotalPages ? getUserTotalPages : 0"
            @click:row="
          ...
          >
            ...
            <template v-slot:[`item.applied_count`]="{ item }">
              <span>{{ item.applied_count ? item.applied_count : '-' }}</span>
            </template>
            <template v-slot:[`item.matched_count`]="{ item }">
              <span>{{ item.matched_count ? item.matched_count : '-' }}</span>
            </template>
            <template v-slot:[`item.cancel_count`]="{ item }">
              <span>{{ item.cancel_count ? item.cancel_count : '-' }}</span>
            </template>
...
export default {
  ...
  created() {
    this.$store.dispatch('DASHBOARDS_GET_ALL', {
      id: this.user.id
    })
    this.getDataFromApi()
  },
      ...
      if (inputSearchType == 'nationalities') {
        ...
        this.searchFields = [
          ...
          },
          {
            ...
            locale: 'ja',
            range: true,
            range_input: 'registration_date_from'
          }
        ]
