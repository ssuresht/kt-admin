...
import InternshipFeatureDialog from '@/components/models/InternshipFeatureDialog'
import SimpleModel from '@/components/models/SimpleModel'
import SearchBox from '@/components/ui/SearchBox'
import { mapGetters } from 'vuex'
export default {
  components: { DataTable, InternshipFeatureDialog, SimpleModel, SearchBox },
  data() {
    return {
      launchEdit: false,
      launchNew: false,
  ...
  computed: {
    ...mapGetters([
      ...
      'getInternshipFeaturesPagination'
    ]),
    totalRecords() {
      return this.getInternshipFeaturesPagination?.records_total || 0
    },
    totalPages() {
      return this.getInternshipFeaturesPagination?.total_pages || 0
    },
    getInternshipFeatures() {
      return this.getAllInternshipFeatures.map(InternshipFeature => {
        return {
  ...
  methods: {
    ...
    async deleteInternshipFeature() {
      await this.$store
        .dispatch('INTERNSHIP_FEATURES_DELETE', this.temporaryDeleteId)
        .then(res => {
          if (res.status === 200) {
    ...
    async getDataFromApi(e = undefined, obj = {}) {
      ...
      await this.$store
        .dispatch('INTERNSHIP_FEATURES_GET_ALL', data)
        .then(res => {
          if (this.initialLoad) {
